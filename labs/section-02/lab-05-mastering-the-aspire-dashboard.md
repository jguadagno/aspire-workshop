...
- `productsapi` state changes to **Finished**
...
3. You should see your structured log entries
4. Click on a log row where the message starts with "*Fetching products list*" to expand it — notice the `RequestId` and `RemoteIP` fields are individually searchable
5. Use the **Filter** box to search for `RemoteIP` — only log entries with that field appear
...
> Note: This is a simplified example, yours may have more spans depending on the number of services and operations involved.
...
1. Back to the **Traces** view, click a `GET /products` trace from `webfrontend` with at least 2 spans (one for `webfrontend` and one for `productsapi`)
2. In a trace for `GET /products`, find the span named something like `SELECT [Products]...` — this is the database query generated by EF Core
3. Click it to expand the attributes panel
4. You'll see:
